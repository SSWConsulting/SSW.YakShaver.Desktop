...
export class MCPOrchestratorNeo {
  ...
  public async processMessageAsync(
    ...
    const mcpServerClients =
      await MCPOrchestratorNeo.mcpServerManager?.getAllMcpServerClientsAsync();
    ...
    const toolSets = await Promise.all(mcpServerClients.map((client) => client.listTools()));
    ...
    const { text, steps, reasoning } = await MCPOrchestratorNeo.llmProvider.sendMessage(
      messages,
      tools,
    );
