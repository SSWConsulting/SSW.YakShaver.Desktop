...
import { MCPServerConfig, validateMcpTransportConfig } from "./types";
...
export class MCPServerManager {
    ...
    public async getAllMcpServerClientsAsync(): Promise<MCPServerClient[] | null> {
        const serverConfigs = MCPServerManager.serverConfigs ?? [];
        const enabledServers = serverConfigs.filter((config) => config.enabled !== false);
        if (enabledServers.length === 0) {
            return [];
        }
        const clientPromises = enabledServers.map((config) =>
    ...
    async addServerAsync(config: MCPServerConfig): Promise<void> {
        ...
        MCPServerManager.serverConfigs.push({
            ...config,
    ...
    async updateServerAsync(name: string, config: MCPServerConfig): Promise<void> {
        ...
        MCPServerManager.serverConfigs[index] = normalizedConfig;
        await this.saveConfigAsync();
    }
