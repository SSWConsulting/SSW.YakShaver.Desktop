...
export function createVideoSource(data: CreateVideoSourceData): VideoSource {
  const result = getDb().insert(videoSources).values(data).returning().get();
  return result;
}
...
export function getVideoSourceById(id: string): VideoSource | undefined {
  return getDb().select().from(videoSources).where(eq(videoSources.id, id)).get();
}
...
export function getAllVideoSources(): VideoSource[] {
  return getDb().select().from(videoSources).orderBy(desc(videoSources.createdAt)).all();
}
...
export function findVideoSourceByUrl(sourceUrl: string): VideoSource | undefined {
  return getDb().select().from(videoSources).where(eq(videoSources.sourceUrl, sourceUrl)).get();
}
...
export function deleteVideoSource(id: string): boolean {
  const result = getDb().delete(videoSources).where(eq(videoSources.id, id)).run();
