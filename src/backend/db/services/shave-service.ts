...
export function createShave(data: CreateShaveData): Shave {
  const result = getDb().insert(shaves).values(data).returning().get();
  return result;
}
...
export function getShaveById(id: string): Shave | undefined {
  return getDb().select().from(shaves).where(eq(shaves.id, id)).get();
}
...
export function getAllShaves(): Shave[] {
  return getDb().select().from(shaves).orderBy(desc(shaves.createdAt)).all();
}
...
export function findShaveByVideoUrl(videoEmbedUrl: string): Shave | undefined {
  return getDb().select().from(shaves).where(eq(shaves.videoEmbedUrl, videoEmbedUrl)).get();
}
...
export function updateShave(id: string, data: UpdateShaveData): Shave | undefined {
  const result = getDb().update(shaves).set(data).where(eq(shaves.id, id)).returning().get();
  return result;
}
...
export function deleteShave(id: string): boolean {
  const result = getDb().delete(shaves).where(eq(shaves.id, id)).run();
